...
function geometry(params) {
    ...
    var data = options.data, sourceField = options.sourceField, targetField = options.targetField, weightField = options.weightField, color = options.color, nodeStyle = options.nodeStyle, edgeStyle = options.edgeStyle, label = options.label, tooltip = options.tooltip, nodeAlign = options.nodeAlign, nodePaddingRatio = options.nodePaddingRatio, nodeWidthRatio = options.nodeWidthRatio, nodeSort = options.nodeSort;
    // 1. 组件，优先设置，因为子 view 会继承配置
    chart.legend(false);
    chart.tooltip(tooltip);
    chart.axis(false);
    // 2. 转换出 layout 前数据
    var sankeyLayoutInputData = data_1.transformDataToNodeLinkData(data, sourceField, targetField, weightField);
    // 3. layout 之后的数据
